...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    // Un token vencido o con permisos insuficientes no debe bloquear archivos
    // públicos. Reintentamos sin Authorization; los privados seguirán dando 401.
    if (upstream.status === 401 && upstreamHeaders.Authorization) {
      const publicHeaders = { ...upstreamHeaders }
  ...
  if (!upstream.ok && upstream.status !== 206) {
    return new Response(
      isHuggingFaceTarget(targetValue) && token
        ? "Unable to access this Hugging Face file with the configured token"
        : "Unable to fetch PDF",
